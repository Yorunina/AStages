...
//    }
    ...
    public static  <W, R extends ARestriction<R, ?, ?>> @Nullable R getRestrictionFromCache(AHolder holder, OrderedMultiMap<W, R> cache, W value) {
        ...
        if (holder.isServerActive()) {
            serverRestriction = getRestrictionFromCache(holder, AStageType.SERVER, cache, value);
        }

        if (holder.isPlayerActive()) {
            playerRestriction = getRestrictionFromCache(holder, AStageType.PLAYER, cache, value);
        }

        // Use getServerAndPlayerRestriction to handle REVERSE modifier correctly (preserves 1.20.X-fix behavior)
        @SuppressWarnings("unchecked")
        R result = (R) getServerAndPlayerRestriction(serverRestriction, playerRestriction);
        return result;
    }
    ...
    public static  <W, R extends ARestriction<R, ?, ?>> @Nullable R getRestrictionFromCache(AHolder holder, IndexedOrderedMultiMap<W, R> cache, W value) {
        ...
        if (holder.isPlayerActive()) {
            playerRestriction = getRestrictionFromCache(holder, AStageType.PLAYER, cache, value);
            if (holder.holdOnlyOneType()) { return playerRestriction; }
        }
